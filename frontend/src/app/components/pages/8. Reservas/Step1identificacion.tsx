...
export function Step1Identificacion({ data, onChange, onNext, onBack }: Step1IdentificacionProps) {
  ...
  // Coverage options — loaded from ms-paciente, fallback to hardcoded
  ...
  // On RUN blur: look up patient in ms-paciente and preload their data
  const handleIdentifierBlur = async () => {
    ...
    try {
      ...
      // Patient not found or ms-paciente unavailable — user fills manually
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4">
        <div>
          <label className="block text-sm font-medium text-[#0b3c5d] mb-2 uppercase tracking-wide text-xs">
            Tipo de Identificación
          </label>
          <div className="flex gap-4">
            {(['RUN', 'PASAPORTE'] as const).map(type => (
              <label key={type} className="flex items-center gap-2 text-sm text-slate-600 font-medium">
                <input
                  type="radio"
                  checked={idType === type}
